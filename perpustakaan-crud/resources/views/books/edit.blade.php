...
<div class="container mx-auto p-4">
    ...
    <form action="{{ route('books.update', $book->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" id="title" name="title" value="{{ $book->title }}" class="w-full border rounded px-4 py-2" required>
        </div>
        <div class="mb-4">
            <label for="author" class="block text-gray-700">Penulis:</label>
            <input type="text" id="author" name="author" value="{{ $book->author }}" class="w-full border rounded px-4 py-2" required>
        </div>
        <div class="mb-4">
            <label for="published_year" class="block text-gray-700">Tahun Terbit:</label>
            <input type="number" id="published_year" name="published_year" value="{{ $book->published_year }}" class="w-full border rounded px-4 py-2" required>
        </div>
        <div class="mb-4">
        ...
        @foreach ($categories as $category)
            <div class="mr-4 mb-2">
                <input type="checkbox" id="category_{{ $category->id }}" name="categories[]" value="{{ $category->id }}" 
                    @if($book->categories->contains($category->id)) checked @endif class="mr-2">
